...
parser.add_option("-f", "--filter", dest="filteredcountry",
	help="Enter Your Country Code To Exclude From Search")
...
if options.filteredcountry:
    try:
        country_list = options.filteredcountry.split(",")
    except:
        print("Country Filter Must Have These Two Format:")
        print("[1] -f \"IR\"")
        print("[2] -f \"IR,JP,US\"")
...
def CountryCheck(ip):
    ...
    return loaded_data["country_code"]
    ...
    try:
        if len(country_list)>0:
            for country in country_list:
                check_c = CountryCheck(ip)
                if check_c.lower() == country.lower():
                    return 4
    except:
        pass
    if IPValidCheck(ip):
...
for ip_addr in input_ips:
    ...
    if check_status==1:
        ...
        print(f"IP Address {ip_addr} Is On Your Filter List.")
